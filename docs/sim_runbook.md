...
- **Stack overlay (config_generation).** New launch arg `hydra_semantic_topic`
  ...
  points hydra at `<robot>_zed/semantic/gt_image_raw`. `spot_isaac` now uses a
  new `map_isaac` launch group (`[main, hydra_isaac, roman]`). Regenerated
  `tmux/autogenerated/spot_isaac-isaac_sim.yaml`. instance_segmentation still
  runs (harmless — hydra no longer subscribes to it; the executor's own YOLOE
  reads the RGB topic directly, so the pick's detection gate is untouched).
